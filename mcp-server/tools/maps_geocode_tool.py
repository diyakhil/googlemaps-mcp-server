...
def maps_geocode(address: str) -> dict:
    try: 
        ...
        return {"success": False, "error": f"Unhandled exception in geocode tool: {e}"}

def maps_reverse_geocode(latitude: float, longitude: float) -> dict: 
    try: 
        result = gmaps.reverse_geocode((latitude, longitude))
        if not result: 
            return {"success": False, "error": "No results found for the provided lat long."}
        return {"success": True, "address": result[0]["formatted_address"]}
    except Exception as e: 
        return {"success": False, "error": f"Unhandled exception: {e}"}
...
def maps_place(place_id: str) -> dict : 
    ...
    try: 
        ...
        result = gmaps.place(place_id = place_id, fields=[ 
            "name","website"])
        if not result: 
            return {"success": False, "error": "No results found for the provided placeId"}
        return result
    except Exception as e: 
        return {"success": False, "error": f"Unhandled exception in place tool: {e}"}
...
def maps_get_directions(origin: str, destination: str, mode='driving', alternatives=True) -> dict: 
    try: 
        ...
        if not result: 
            return {"success": False, "error": "No results found for the provided origin and destination"}
        instructions = ". ".join([step["html_instructions"] for step in steps])
        return {"directions": instructions}
    except Exception as e: 
        return {"success": False, "error": f"Unhandled exception in get directions tool: {e}"}
...
def maps_travel_time(origin: str, destination: str, mode='driving') -> dict:
    try:
        ...
        return {"success": False, "error": f"Unhandled exception in travel_time tool: {e}"}